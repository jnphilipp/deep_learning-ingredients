    ...
    **kwargs,
    ...
    for i in range(blocks):
        for j, x in enumerate(xs):
            ...
            x = block(
                x,
                do_pooling=i != blocks - 1,
                filters=filters,
                nb_filters=x._shape_val[-1],
                shortcuts=shortcuts,
    ...
    # outputs
    outs, loss, metrics = (
        outputs(xs, outputs=kwargs["outputs"], layers=layers, shortcuts=shortcuts)
        if "outputs" in kwargs
        else outputs(xs, shortcuts=shortcuts)
    )
...
def layer(
    ...
    if bottleneck2d is not None:
        ...
        x = Conv2D.from_config(dict(bottleneck2d, **{"filters": factor * k}))(x)
        if batchnorm is not None:
