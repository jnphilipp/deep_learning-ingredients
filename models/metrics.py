...
from tensorflow.keras.metrics import (Accuracy, AUC, BinaryAccuracy,
                                      BinaryCrossentropy, CategoricalAccuracy,
                                      CategoricalCrossentropy,
                                      CategoricalHinge, CosineSimilarity,
                                      FalseNegatives, FalsePositives, Hinge,
                                      KLDivergence, LogCoshError, Mean,
                                      MeanAbsoluteError,
                                      MeanAbsolutePercentageError, MeanIoU,
                                      MeanRelativeError, MeanSquaredError,
                                      MeanSquaredLogarithmicError, MeanTensor,
                                      Metric, Poisson, Precision,
                                      ...
                                      SpecificityAtSensitivity, SquaredHinge,
                                      Sum, TopKCategoricalAccuracy,
...
@ingredient.capture
def get(class_names: List[str], _log: Logger, **kwargs) -> List[Metric]:
    metrics = []
...
def accuracy(_log: Logger, name: str = 'accuracy',
             dtype: Optional[Any] = None) -> Accuracy:
...
def auc(_log: Logger, num_thresholds: int = 200, curve: str = 'ROC',
        summation_method: str = 'interpolation', name: Optional[str] = None,
        dtype: Optional[Any] = None, thresholds: Optional[List[float]] = None,
        multi_label: bool = False, label_weights: Optional[Any] = None) -> AUC:
    ...
    _log.debug(f'Config: num_thresholds={num_thresholds}, curve={curve}, ' +
    ...
    return AUC(num_thresholds, curve, summation_method, name, dtype,
               thresholds, multi_label, label_weights)


@ingredient.capture(prefix='binary_accuracy')
def binary_accuracy(_log: Logger, name: str = 'binary_accuracy',
                    dtype: Optional[Any] = None, threshold: float = 0.5) -> \
        BinaryAccuracy:
...
def binary_crossentropy(_log: Logger, name: str = 'binary_crossentropy',
                        dtype: Optional[Any] = None, from_logits: bool = False,
                        label_smoothing: float = 0.) -> BinaryCrossentropy:
...
def categorical_accuracy(_log: Logger, name: str = 'categorical_accuracy',
                         dtype: Optional[Any] = None) -> CategoricalAccuracy:
...
def categorical_crossentropy(_log: Logger,
...
def categorical_hinge(_log: Logger, name: str = 'categorical_hinge',
                      dtype: Optional[Any] = None) -> CategoricalHinge:
...
def cosine_similarity(_log: Logger, name: str = 'cosine_similarity',
                      dtype: Optional[Any] = None, axis: int = -1) -> \
        CosineSimilarity:
...
def false_negatives(_log: Logger, thresholds: Union[float, List[float]] = None,
                    name: Optional[str] = None,
                    dtype: Optional[Any] = None) -> FalseNegatives:
...
def false_positives(_log: Logger, thresholds: Union[float, List[float]] = None,
                    name: Optional[str] = None,
                    dtype: Optional[Any] = None) -> FalsePositives:
...
def hinge(_log: Logger, name: str = 'hinge', dtype: Optional[Any] = None) -> \
        Hinge:
...
def kullback_leibler_divergence(_log: Logger,
                                name: str = 'kullback_leibler_divergence',
                                dtype: Optional[Any] = None) -> KLDivergence:
...
def logcosh(_log: Logger, name: str = 'logcosh',
            dtype: Optional[Any] = None) -> LogCoshError:
...
def mean(_log: Logger, name: str = 'mean', dtype: Optional[Any] = None) -> \
        Mean:
...
def mean_absolute_error(_log: Logger, name: str = 'mean_absolute_error',
                        dtype: Optional[Any] = None) -> MeanAbsoluteError:
...
def mean_absolute_percentage_error(_log: Logger,
                                   name: str = 'mean_absolute_percentage_' +
                                   'error', dtype: Optional[Any] = None) -> \
...
def meaniou(_log: Logger, num_classes: int, name: Optional[str] = None,
            dtype: Optional[Any] = None) -> MeanIoU:
    _log.info('MeanIoU metric.')
    _log.debug(f'Config: num_classes={num_classes} name={name}, ' +
               f'dtype={dtype}.')
    return MeanIoU(num_classes, name, dtype)


@ingredient.capture(prefix='mean_relative_error')
def mean_relative_error(_log: Logger, normalizer: Any,
                        name: Optional[str] = None,
                        dtype: Optional[Any] = None) -> MeanRelativeError:
...
def mean_squared_error(_log: Logger, name: str = 'mean_squared_error',
                       dtype: Optional[Any] = None) -> MeanSquaredError:
...
def mean_squared_logarithmic_error(_log: Logger,
                                   name: str = 'mean_squared_logarithmic_' +
                                   'error', dtype: Optional[Any] = None) -> \
...
def mean_tensor(_log: Logger, name: str = 'mean_tensor',
                dtype: Optional[Any] = None) -> MeanTensor:
...
def poisson(_log: Logger, name: str = 'poisson',
            dtype: Optional[Any] = None) -> Poisson:
...
def precision(_log: Logger,
              thresholds: Optional[Union[float, List[float]]] = None,
              top_k: Optional[int] = None, class_id: Optional[int] = None,
              name: Optional[str] = None, dtype: Optional[Any] = None) -> \
        Precision:
    _log.info('Precision metric.')
    _log.debug(f'Config: thresholds={thresholds}, top_k={top_k}, ' +
...
def precision_at_recall(_log: Logger, recall: float, num_thresholds: int = 200,
                        name: Optional[str] = None,
                        dtype: Optional[Any] = None) -> PrecisionAtRecall:
    _log.info('PrecisionAtRecall metric.')
    _log.debug(f'Config: recall={recall}, num_thresholds={num_thresholds}, ' +
               f'name={name}, dtype={dtype}.')
    return PrecisionAtRecall(recall, num_thresholds, name, dtype)


@ingredient.capture(prefix='recall')
def recall(_log: Logger,
           thresholds: Optional[Union[float, List[float]]] = None,
           top_k: Optional[int] = None, class_id: Optional[int] = None,
           name: Optional[str] = None, dtype: Optional[Any] = None) -> Recall:
...
def root_mean_squared_error(_log: Logger,
...
def sensitivity_at_specificity(_log: Logger, specificity:  float,
                               num_thresholds: int = 200,
                               name: Optional[str] = None,
                               dtype: Optional[Any] = None) -> \
        SensitivityAtSpecificity:
    _log.info('SensitivityAtSpecificity metric.')
    _log.debug(f'Config: specificity={specificity}, ' +
...
def sparse_categorical_accuracy(_log: Logger,
...
def sparse_categorical_crossentropy(_log: Logger,
...
def sparse_top_k_categorical_accuracy(_log: Logger, k: int = 5,
                                      name: str = 'sparse_top_k_categorical_' +
                                      'accuracy',
                                      dtype: Optional[Any] = None) -> \
        SparseTopKCategoricalAccuracy:
...
def specificity_at_sensitivity(_log: Logger, sensitivity:  float,
                               num_thresholds: int = 200,
                               name: Optional[str] = None,
                               dtype: Optional[Any] = None) -> \
        SpecificityAtSensitivity:
    _log.info('SpecificityAtSensitivity metric.')
    _log.debug(f'Config: sensitivity={sensitivity}, ' +
               f'num_thresholds={num_thresholds}, name={name}, dtype={dtype}.')
    return SpecificityAtSensitivity(sensitivity, num_thresholds, name, dtype)


@ingredient.capture(prefix='squared_hinge')
def squared_hinge(_log: Logger, name: str = 'squared_hinge',
                  dtype: Optional[Any] = None) -> SquaredHinge:
...
def top_k_categorical_accuracy(_log: Logger, k: int = 5,
...
def true_negatives(_log: Logger,
                   thresholds: Optional[Union[float, List[float]]] = None,
                   name: Optional[str] = None,
                   dtype: Optional[Any] = None) -> TrueNegatives:
...
def true_positives(_log: Logger,
                   thresholds: Optional[Union[float, List[float]]] = None,
                   name: Optional[str] = None,
                   dtype: Optional[Any] = None) -> TruePositives:
