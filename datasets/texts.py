    ...
    def __getitem__(self, idx: int) -> Union[Tuple[Dict[str, np.ndarray],
        ...
        if self.mode == 'separate':
            ...
            for k in self.X.keys():
                bx[k] = np.zeros((current_batch_size, self.X[k][0]),
                                 dtype=self.dtype)
            by = {}
            for k in self.Y.keys():
                by[k] = np.zeros((current_batch_size, self.Y[k][0]),
                                 dtype=self.dtype)

            if self.sample_weights:
                bw = {}
                for k in self.X.keys():
                    bw[k] = np.zeros((current_batch_size, self.X[k][0]),
                                     dtype=np.uint)
            ...
            y = np.zeros((current_batch_size,
                          sum([v[0] for v in self.Y.values()])),
                         dtype=self.dtype)
            if self.sample_weights:
                w = np.zeros((current_batch_size,
                              sum([v[0] for v in self.X.values()])),
                             dtype=np.uint)
            for i, j in enumerate(range(start_idx, end_idx)):
...
@ingredient.capture
def get(dataset: str, batch_size: int, mode: str,
        ...
        vocab_path: Optional[str] = None) -> Union[TextSequence,
                                                   Tuple[TextSequence,
                                                         TextSequence]]:
    assert mode in ['separate', 'concat']
    assert validation_split is None or (validation_split >= 0. and
                                        validation_split <= 1.)
    ...
    for k in tx.keys():
        ...
        train_x[k if k.startswith('input_') else f'input_{k}'] = (max_len,
                                                                  tx[k])
    train_y = {}
    for k in ty.keys():
        max_len = max([len(i) for i in ty[k]])
        train_y[k if k.startswith('output_') else f'output_{k}'] = (max_len,
                                                                    ty[k])
    ...
    if os.path.exists(val_csv):
        ...
        for k in tx.keys():
            ...
            val_x[k if k.startswith('input_') else f'input_{k}'] = (max_len,
                                                                    vx[k])
        val_y = {}
        for k in ty.keys():
            max_len = max(train_y[k][0], max([len(i) for i in ty[k]]))
            train_y[k] = (max_len, train_y[k][1])
            val_y[k if k.startswith('output_') else f'output_{k}'] = (max_len,
                                                                      vy[k])
        ...
        if validation_split:
            ...
            for k in train_x.keys():
                val_x[k] = (train_x[k][0],
                            [train_x[k][1][i] for i in per[idx:]])
                train_x[k] = (train_x[k][0],
                              [train_x[k][1][i] for i in per[0:idx]])
            val_y = {}
            for k in train_y.keys():
                val_y[k] = (train_y[k][0],
                            [train_y[k][1][i] for i in per[idx:]])
